      ...
      $scope.login = function () {
        ...
        $scope.loginResult = User.login({
          ...
          function (user) {
            ...
            //var next = $location.nextAfterLogin || '/'; //it was with defaui change so that it goes to admin panel
            var next = $location.nextAfterLogin || '/admin'; //
            $location.nextAfterLogin = null;
            AppAuth.currentUser = $scope.loginResult.user;
            CoreService.toastSuccess(gettextCatalog.getString('Logged in'),
              gettextCatalog.getString('You are logged in!'));
            if (next === '/login') {
              //next = '/'; //it was with default repository
              next = '/admin'; // i change so that when people login, it goes to admin panel
            }
            $location.path(next);
