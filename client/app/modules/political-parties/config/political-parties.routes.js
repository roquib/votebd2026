    ...
    .config(function ($stateProvider) {
      $stateProvider
        ...
        .state('app.politicalParties.add', {
          ...
          controllerAs: 'ctrl',
          controller: function ($scope, $state, PoliticalPartiesService, politicalParty, Upload, $timeout) {
            $scope.uploadPic = function(politicalPartyMarka) {
              ...
              politicalPartyMarka.upload.then(function (response) {
                $timeout(function () {
                ...
                politicalPartyMarka.progress = Math.min(100, parseInt(100.0 * evt.loaded / evt.total));
              });
            }
            ...
            this.submit = function () {
              this.politicalParty.politicalPartyMarka=$scope.politicalPartyMarka;
              PoliticalPartiesService.upsertPoliticalParty(this.politicalParty).then(function () {
                $state.go('^.list');
              });
            };
          },
        ...
        .state('app.politicalParties.delete', {
          ...
          template: '',
          controllerAs: 'ctrl',
          controller: function ($state, PoliticalPartiesService, politicalParty) {
            PoliticalPartiesService.deletePoliticalParty(politicalParty.id, function () {
              $state.go('^.list');
            }, function () {
              $state.go('^.list');
            });
