(function () {
  ...
  angular
    ...
    .service('CandidateComparisonService', function (CoreService, Candidate, $filter, gettextCatalog) {
      ...
      this.getCandidatesWhere = function (electionSeatCriteria) { //console.log(electionSeatCriteria);

        var whereCriteria = this.verifyFilterData(electionSeatCriteria);
        return Candidate.find(
            {
              filter: {
                //fields: ['professionTypeBnAF'],
                limit: 1000,
                where: whereCriteria,
                include: ["politicalParty","currentElection","electionSeat"]
              }
          }
        ).$promise;
      };

      this.differentElectionParticipateCandidate=function(){

        return Candidate.differentElectionParticipateCandidate().$promise;
      }
