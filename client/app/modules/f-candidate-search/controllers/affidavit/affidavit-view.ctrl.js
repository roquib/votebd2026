(function () {
  ...
  angular
    ...
    .controller('CandidateSearchAffidavitCtrl', function($scope, $stateParams,Candidate, CandidateSearchService) {
      this.candidateId = $stateParams.id;
      //this.profileType = $stateParams.profileType;
      this.candidat=Candidate.findById({id:this.candidateId, filter:{include:['politicalParty','electionSeat','currentElection']}});
      console.log(this.candidat);
      $scope.candidates=[];
      CandidateSearchService.getPersonId(this.candidateId).then(function(info){
        CandidateSearchService.getCandidateInformation(info[0].personId).then(function(candidate){
