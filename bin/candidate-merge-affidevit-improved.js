...
//oldds.connector.query("SELECT * FROM effidevit WHERE candidate_id<>0 AND publish='publish' limit 10000,5000", '', function (err, rows, fields) {
//  if (err) {
//    console.log(err);
//    throw err;
//  }
...
//      candidateAffidevit.oldAffidevitId = eachRow.id;
//      candidateAffidevit.statusAF = eachRow.publish;
...
//      candidateAffidevit.degreeTypeBnAF = eachRow.degree_type;
...
//        candidateAffidevit.pastCriminalCourtStatusAF = false; //check ans save 0/1
//      else
//        candidateAffidevit.pastCriminalCourtStatusAF = true; //check ans save 0/1
//
//      candidateAffidevit.criminalCourtStatusBnAF = eachRow.criminal_court_status;
//      if (eachRow.criminal_court_status == "হ্যাঁ")
//        candidateAffidevit.criminalCourtStatusAF = false; //check ans save 0/1
//      else
//        candidateAffidevit.criminalCourtStatusAF = true; //check ans save 0/1
//
...
//        candidateAffidevit.isPastMpAF = true; //check and save 0/1
//      else
//        candidateAffidevit.isPastMpAF = false; //check and save 0/1
//
//      comitbefore = {};
//      arr = [];
//      if (eachRow.commitment_before_mp) {
//        comitbefore = PHPUnserialize.unserialize(eachRow.commitment_before_mp); //need to save as array
//        if(comitbefore){
//          arr = Object.keys(comitbefore).map(function (k) {
//            return comitbefore[k]
//          });
//          candidateAffidevit.commitmentBeforeMpAF = arr;
//        }
//      }
//
//      comitwhile = {};
...
//        comitwhile = PHPUnserialize.unserialize(eachRow.achievement_while_mp); //need to save as array
//        if(comitwhile){
//          arr2 = Object.keys(comitwhile).map(function (k) {
//            return comitwhile[k]
//          });
//          candidateAffidevit.achievementWhileMpAF = arr2;
//        }
//      }
//
//
...
oldds.connector.query("SELECT profile_id, COUNT(id) as cnt FROM `effidevit_loans_section` where publish='publish' group by profile_id limit 0, 4000",'',function(err,rows,fields) {
  if(err){
    console.log(err);
    throw err;
  }
      ...
      oldds.connector.query("SELECT * FROM `effidevit_loans_section` where profile_id="+eachRow.profile_id,'',function(err22,rows2,fields) {
        ...
        if(profilesLen){
          ...
          for(var i=0;i<profilesLen;i++){
            ...
            loanAmount=0;
            if(IsNumeric(rows2[i].loan_amount))
              loanAmount=Number(rows2[i].loan_amount)

            totalSingleAmountAF=0;
            if(rows2[i].loan_type=='single' && loanAmount)
              totalSingleAmountAF =loanAmount;

            totalJointAmoutAF=0;
            if(rows2[i].loan_type=='joint' && loanAmount)
              totalJointAmoutAF =loanAmount;

            totalDependedantsAmountAF=0;
            if(rows2[i].loan_type=='dependants' && loanAmount)
              totalDependedantsAmountAF =loanAmount;

            totalDirectorOrChairmenAmoutAF=0;
            if(rows2[i].loan_type=='director_chairman' && loanAmount)
          ...
          candidateAffidevit.totalSingleAmountAF = totalSingleAmountAF;
          candidateAffidevit.totalJointAmoutAF = totalJointAmoutAF;
          candidateAffidevit.totalDependedantsAmountAF = totalDependedantsAmountAF;
          candidateAffidevit.totalDirectorOrChairmenAmoutAF = totalDirectorOrChairmenAmoutAF;
...
//oldds.connector.query("SELECT profile_id, COUNT(id) as cnt FROM effidevit_income_section where publish='publish'  group by profile_id limit 10000, 5000",'',function(err,rows,fields) {
//  if(err){
//    console.log(err);
//    throw err;
//  }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//oldds.connector.query("SELECT profile_id, COUNT(id) as cnt FROM effidevit_liabilities_section where publish='publish'  group by profile_id limit 10000, 5000", '', function (err, rows, fields) {
//  if (err) {
//    console.log(err);
//    throw err;
//  }
...
//                matAsset.priceHusbandWife = Number(rows2[i].amount_husband_wife);
...
//                matAsset.priceDependants = Number(rows2[i].amount_dependants);
...
//
//              if (rows2[i].amount_husband_wife && IsNumeric(rows2[i].amount_husband_wife) && Number(rows2[i].amount_husband_wife)) {
//                iMatAsset.priceHusbandWife = Number(rows2[i].amount_husband_wife);
...
//
//              if (rows2[i].amount_dependants && IsNumeric(rows2[i].amount_dependants) && Number(rows2[i].amount_dependants)) {
//                iMatAsset.priceDependants = Number(rows2[i].amount_dependants);
...
//                iMatAsset.priceJointOwnership = Number(rows2[i].joint_ownership);
...
//                iMatAsset.priceJointSharePart = Number(rows2[i].joint_share_part);
